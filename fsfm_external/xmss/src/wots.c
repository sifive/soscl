 ...
 * Writes the computed public key to 'pk'.
 */
void wots_pk_from_sig(const xmss_params *params, unsigned char *pk, const unsigned char *sig, const unsigned char *msg, const unsigned char *pub_seed, uint32_t addr[8])
{
    int lengths[params->wots_len];
    uint32_t i;

    chain_lengths(params, lengths, msg);
    printf("/%d %d\n",params->wots_len,params->wots_len*params->n);
    for (i = 0; i < params->wots_len; i++) {
      printf("%d\n",i);
        set_chain_addr(addr, i);
