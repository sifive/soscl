...
void old_addr_to_bytes(unsigned char *bytes, const uint32_t addr[8])
{
    int i;
    for (i = 0; i < 8; i++) {
...
void addr_to_bytes(unsigned char *bytes, const uint32_t addr[8])
{
    ...
    bytes[1*4+3]=addr[1];
    bytes[1*4+2]=addr[1]>>8;
    bytes[1*4+1]=addr[1]>>16;
    bytes[1*4+0]=addr[1]>>24;
    
    bytes[2*4+3]=addr[2];
    bytes[2*4+2]=addr[2]>>8;
    bytes[2*4+1]=addr[2]>>16;
    bytes[2*4+0]=addr[2]>>24;

    bytes[3*4+3]=addr[3];
    bytes[3*4+2]=addr[3]>>8;
    bytes[3*4+1]=addr[3]>>16;
    bytes[3*4+0]=addr[3]>>24;

    bytes[4*4+3]=addr[4];
    bytes[4*4+2]=addr[4]>>8;
    bytes[4*4+1]=addr[4]>>16;
    bytes[4*4+0]=addr[4]>>24;

    bytes[5*4+3]=addr[5];
    bytes[5*4+2]=addr[5]>>8;
    bytes[5*4+1]=addr[5]>>16;
    bytes[5*4+0]=addr[5]>>24;
    
    bytes[6*4+3]=addr[6];
    bytes[6*4+2]=addr[6]>>8;
    bytes[6*4+1]=addr[6]>>16;
    bytes[6*4+0]=addr[6]>>24;

    bytes[7*4+3]=addr[7];
    bytes[7*4+2]=addr[7]>>8;
    bytes[7*4+1]=addr[7]>>16;
    bytes[7*4+0]=addr[7]>>24;

    /*	for (i = 0; i < 8; i++)
      {
	bytes[i*4+3]=addr[i];
	bytes[i*4+2]=addr[i]>>8;
	bytes[i*4+1]=addr[i]>>16;
	bytes[i*4+0]=addr[i]>>24;
	//        ull_to_bytes(bytes + i*4, 4, addr[i]);
	}*/
}
...
static int core_hash(const xmss_params *params, unsigned char *out, const unsigned char *in, unsigned long long inlen)
{
    ...
    if (params->n == 32 && params->func == XMSS_SHA2)
      {
	hca_sha256(in, inlen, out);
      }
    else
      if (params->n == 24 && params->func == XMSS_SHA2)
	{
	  hca_sha256(in, inlen, buf);
	  memcpy(out, buf, 24);
	}/*
...
int prf(const xmss_params *params, unsigned char *out, const unsigned char in[32], const unsigned char *key)
{
    ...
    memset(buf,0,params->padding_len-1);
    buf[params->padding_len-1]=XMSS_HASH_PADDING_PRF;
            ...
            const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    memset(buf,0,params->padding_len-1);
    buf[params->padding_len-1]=XMSS_HASH_PADDING_H;
    ...
    //addr_to_bytes(addr_as_bytes, addr);
    addr_as_bytes[7*4+3]=1;
    addr_as_bytes[7*4+2]=0;
    addr_as_bytes[7*4+1]=0;
    addr_as_bytes[7*4+0]=0;
    prf(params, bitmask, addr_as_bytes, pub_seed);

    //    set_key_and_mask(addr, 2);
    //addr_to_bytes(addr_as_bytes, addr);
    addr_as_bytes[7*4+3]=2;
    addr_as_bytes[7*4+2]=0;
    addr_as_bytes[7*4+1]=0;
    addr_as_bytes[7*4+0]=0;
...
//for signature verification, this is mainly where the time is spent
int thash_f(const xmss_params *params, unsigned char *out, const unsigned char *in, const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    memset(buf,0,params->padding_len);
