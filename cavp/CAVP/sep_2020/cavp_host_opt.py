...
#        outwfile.write(st)
        if received == output:
            ...
            #for gcm decrypt, the received answer contains the result and the tag verification, that may be false
            #but the expected answer can be false only (no result if the tag verif is false)
            #so a specific process is required
            #(note: for ccm, the result is always expected, even the tag verif is false, which is simpler)
            if modeofoperation == "gcm":
                if("false" in output) and ("false" in received):
                    print("TEST VECTOR OK\n");
                    st="TEST VECTOR "+test+" OK\n"
                    outwfile.write(st)
                    continue
